...
public class UnityGCMIntentService extends GCMBaseIntentService {
	...
	private static final String TAG = UnityGCMIntentService.class
			.getSimpleName();
	...
	protected void onMessage(Context context, Intent intent) {
		...
		// Show native notification view in status bar if defined fields are
		// put.
		String contentTitle;
		try {
			...
			UnityGCMNotificationManager.showNotification(this, contentTitle,
					contentText, ticker);
		} catch (JSONException e) {
			// Title is mandatory, do not display in status bar
			Log.v(TAG,
					"No content_title specified, not showing anything in Android status bar");
		}
	}
